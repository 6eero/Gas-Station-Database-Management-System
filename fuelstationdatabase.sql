...
$$
BEGIN
	PERFORM * FROM AZIENDA AS A, STAZIONE_DI_RIFORNIMENTO AS S1 WHERE A.codice=S1.codiceAzienda and A.codice=new.codiceAzienda;
	IF FOUND THEN 
...
$$
BEGIN
	PERFORM * FROM STAZIONE_DI_RIFORNIMENTO AS S, PIANO_DI_LAVORO_GIORNALIERO AS PDG1 where S.codice=PDG1.codiceStazione and PDG1.codiceStazione=new.codiceStazione;
	IF FOUND THEN 
	    PERFORM * FROM PIANO_DI_LAVORO_GIORNALIERO AS PDG WHERE PDG.codiceStazione=old.codiceStazione AND (PDG.CFDipendente<>old.CFDipendente OR PDG.giorno<>old.giorno);
		if NOT FOUND then 
			RAISE NOTICE 'ogni stazione possiede almeno un piano di lavoro giornaliero ';
...
$$
BEGIN
	PERFORM * FROM STAZIONE_DI_RIFORNIMENTO AS S1, TIPO1 AS T1 WHERE S1.codice=new.codiceStazione AND T1.codiceStazione=S1.codice;
	IF FOUND THEN 
...
$$
BEGIN
	PERFORM * FROM AZIENDA AS A, TIPO1 AS T1 WHERE A.codice=T1.codiceAzienda and A.codice=new.codiceAzienda;
	IF FOUND THEN 
...
$$
BEGIN
	PERFORM * FROM AZIENDA AS A, TIPO2 AS T2 WHERE A.codice=T2.codiceAzienda and A.codice=new.codiceAzienda;
	IF FOUND THEN 
...
$$
BEGIN
	PERFORM * FROM STAZIONE_DI_RIFORNIMENTO AS S, POMPA AS P1 WHERE S.codice=P1.codiceStazione AND S.codice=new.codiceStazione;
	IF FOUND THEN
...
$$
BEGIN
	PERFORM * FROM PIANO_DI_LAVORO_SETTIMANALE AS PDS, PIANO_DI_LAVORO_GIORNALIERO AS PDG1 where PDS.numeroSettimana=PDG1.numeroSettimana and PDG1.numeroSettimana=new.numeroSettimana;
	IF FOUND THEN 
	    PERFORM * FROM PIANO_DI_LAVORO_GIORNALIERO AS PDG WHERE PDG.numeroSettimana=old.numeroSettimana AND (PDG.CFDipendente<>old.CFDipendente OR PDG.giorno<>old.giorno OR PDG.codiceStazione<>old.codiceStazione);
		if NOT FOUND then 
			RAISE NOTICE 'ogni piano di lavoro settimanale possiede almeno un piano di lavoro giornaliero ';
...
$$
BEGIN
	PERFORM * FROM STAZIONE_DI_RIFORNIMENTO AS S, FORNISCE AS F, CARBURANTE AS T where S.codice=F.codiceStazione and F.tipoCarburante=T.tipo and F.codiceStazione=new.codiceStazione;
	if FOUND then 
...
$$
BEGIN
	PERFORM * FROM POMPA AS P, EROGA AS E, CARBURANTE AS T where P.numero=E.numeroPompa and E.codiceStazione=P.codiceStazione and E.tipoCarburante=T.tipo and T.tipo=new.tipoCarburante;
	if FOUND then 
